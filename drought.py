...
def indices(options):
	...
	if selected_indices == "ndvi anomaly":
		...
		return data
		#except ee.EEException as e :
		#	data={'error':'Failed to Compute STANDARDIZED PRECIPITATION INDEX  Data . Error Stated::, '+str(e)}
		#	return data
...
# this is the ish
		...
		SPI_exp=ee.Image(H).expression('(b <0 || b<= 0.5) ? -1*(t1- (c0+c1*t1+c2*t1**2)/(1+d1*t1+d2*t1**2+d3*t1**3))  ' +
	  ':(b <0.5 || b<= 1)? (t2- (c0+c1*t2+c2*t2**2)/(1+d1*t2+d2*t2**2+d3*t2**3)) : null',{
