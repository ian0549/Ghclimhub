...
var palete = {
	'ndvi_anomaly': ["87000A", "7C3E28", "EC712C", "FABF45", "FFFFFF", "51FF78", "3DCF4C", "215229", "0B260B"], 'ndwi_anomaly': ['#b2182b', '#ef8a62', '#fddbc7', '#f7f7f7', '#d1e5f0', '#67a9cf', '#2166ac'], 'vhi': ['#b35806', '#e08214', '#fdb863', '#fee0b6', '#d8daeb', '#b2abd2', '#8073ac', '#542788'], 'smi': ['#730000', '#E60000', '#FFAA00', '#FCD37F', '#FFFF00', '#FFFFFF', '#AAFF55', '#00FFFF', '#00AAFF', '#0000FF', '#0000AA'], 'lst': ["0000ff", "32cd32", "ffff00", "ff8c00", "#e20000 "], 'precipitation': ['#d73027', '#f46d43', '#fdae61', '#fee090', '#e0f3f8', '#abd9e9', '#74add1', '#4575b4'], 'spi': ['#730000', '#E60000', '#FFAA00', '#FCD37F', '#FFFF00', '#FFFFFF', '#AAFF55', '#00FFFF', '#00AAFF', '#0000FF', '#0000AA']
}
...
$(document).on('submit', '#indices_compute_form', function (e) {
	...
	$.ajax({
					...
					if (data.mapid!=null) {
						...
						if (data.type == 'ndvi_anomaly') {
							...
							qScale = d3.scale.quantile()
								.domain(['Extremely Dry', 'Severely Dry','Moderately Dry','Near Normal','Moderately Wet','Very Wet','Extremely Wet'])
								.range(redBlueScale);
		...
		error: function (data) {
			...
			waitingDialog.show('Something went wrong. Could not compute data...');
			setTimeout(function () {
				waitingDialog.hide();
			}, 8000);
